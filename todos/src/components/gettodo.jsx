  ...
  return <>
        {todos.map(e=><div ><p>{e.title}</p><p>{e.status?<Donebtn onClick={()=>changestatus(e)}>{e.status ? "Done":"Not Done"}</Donebtn>:<Yellowbtn onClick={()=>{changestatus(e)}}>{e.status ? "Done":"Not Done"}</Yellowbtn>}</p>
           <p>
            ...
            <Delbtn><Link to={`/todos/${e.id}`} style={{"textDecoration":"none", "color":"#282C34"}}>Explore</Link></Delbtn>
            <Delbtn><Link to={`/todos/${e.id}/edit`} style={{"textDecoration":"none", "color":"#282C34"}}>Edit</Link></Delbtn>
            </p>
